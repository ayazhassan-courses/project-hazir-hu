            ...
            return ListTile(
              title: Text(
                ...
                style: TextStyle(
                    ...
                    fontSize: 16,
                    color: snapshot.data.documents[index]['notification']['title']=='Absent Notification' ? kPrimaryColor : kAccentColor
                ),
              ),
              subtitle: Text(
              ...
              onTap: (){
                for(var course in changeAttendanceprovider.coursedata){
                  if(course.coursename==snapshot.data.documents[index]['data']['coursename']){
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (BuildContext context) => CoursePage(coursedata: course,highlightedDate: snapshot.data.documents[index]['data']['date'],)));
                  }
                }

              },
              leading: snapshot.data.documents[index]['notification']['title']=='Absent Notification' ? Icon(Icons.notification_important,color: kPrimaryColor,) : Icon(Icons.done,color: kAccentColor,)
            );
